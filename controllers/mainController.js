...
const login = async (req, res, next) => {
  ...
  const result_data = data.docs[0].data()
  if (data.empty) {
...
const getrecommend = async (req, res, next) => {
  try {
    ...
    if (data.empty) {
      ...
      data.forEach(doc => {
        const fitness = new ClassFitnessRecommend(
          doc.id,
          doc.data().idfitness,
          doc.data().classname,
          doc.data().amount,
...
const updateNameSubClass = async (req, res, next) => {
  try {
    const id = req.params.id
    const data = req.body
...
const updateTimeSubClass = async (req, res, next) => {
  try {
    const id = req.params.id
    const data = req.body
    const resultdata = await firestore.collection('subclass').doc(id)
